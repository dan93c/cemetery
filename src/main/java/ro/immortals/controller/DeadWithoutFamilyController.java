...
public class DeadWithoutFamilyController extends MainController {
	...
	public ModelAndView list(@PathVariable Integer page,
		...
		modelAndView.addObject(DEADS, deadWithoutFamilyService
	...
	public ModelAndView add(@ModelAttribute DeadWithoutFamily deadWithoutFamily) {
		...
		ModelAndView modelAndView = new ModelAndView(
				ADD_DEAD_WITHOUT_FAMILY_JSP);
		if (deadWithoutFamily == null) {
			modelAndView
					.addObject(DEAD_WITHOUT_FAMILY, new DeadWithoutFamily());
		} else {
	...
	public ModelAndView doAdd(
			@ModelAttribute @Validated DeadWithoutFamily deadWithoutFamily,
		...
		if (!graveService.checkGraveExistence(deadWithoutFamily.getGrave(),
				plotId, cemeteryId)) {
			ModelAndView modelAndView = new ModelAndView(
					ADD_DEAD_WITHOUT_FAMILY_JSP);
			modelAndView.addObject(ERROR_MESSAGE, messageSource.getMessage(
					"message.grave.not.exists",
					new Object[] { deadWithoutFamily.getGrave().getNrGrave() },
					Locale.getDefault()));
			modelAndView.addObject(DEAD_WITHOUT_FAMILY, deadWithoutFamily);
			modelAndView.addObject(CEMETERIES, cemeteryService.getAll());
			modelAndView.addObject(PLOTS, plotService.getAll());
	...
	public ModelAndView edit(@PathVariable Integer id) {
		ModelAndView modelAndView = new ModelAndView(
				EDIT_DEAD_WITHOUT_FAMILY_JSP);
		modelAndView.addObject(DEAD_WITHOUT_FAMILY,
				deadWithoutFamilyService.getById(id));
		modelAndView.addObject(CEMETERIES, cemeteryService.getAll());
		modelAndView.addObject(PLOTS, plotService.getAll());
		modelAndView.addObject(GRAVES, graveService.getAll());
		return modelAndView;
	...
	public ModelAndView doEdit(
		...
		if (!graveService.checkGraveExistence(deadWithoutFamily.getGrave(),
				deadWithoutFamily.getGrave().getPlot().getId(),
				deadWithoutFamily.getGrave().getPlot().getCemetery().getId())) {
			ModelAndView modelAndView = new ModelAndView(
					EDIT_DEAD_WITHOUT_FAMILY_JSP);
			modelAndView.addObject(ERROR_MESSAGE, messageSource.getMessage(
					"message.grave.not.exists",
