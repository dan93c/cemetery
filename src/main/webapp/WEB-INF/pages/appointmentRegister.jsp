...
<title>Registrul anual de programare a inmormantarilor</title>
</head>
<body>
	...
	<div id="container">
		...
		<div id="search_filter_container">
			<div id="filter_container">
				<form method="get"
					action="${CONTEXT_PATH}/dead/inmormantari/${currentPage}?order=${order}&sch=${sch}"
			...
			<ul id="pagin">
				...
				<c:choose>
					<c:when test="${currentPage != 1}">
						<li title="First page"><a
							href="${CONTEXT_PATH}/dead/inmormantari/1?order=${order}&sch=${sch}">
								First </a></li>
						<li title="Previous page"><a
							href="${CONTEXT_PATH}/dead/inmormantari/${currentPage - 1}?order=${order}&sch=${sch}">Previous</a></li>
					</c:when>
					<c:otherwise>
						...
						<li class="disabled" title="Previous page"><a
							href="${CONTEXT_PATH}/dead/inmormantari/1?order=${order}&sch=${sch}">Previous</a></li>
					</c:otherwise>
				</c:choose>
				<c:forEach begin="${begin}" end="${end}" var="i">
					<c:choose>
						...
						<c:otherwise>
							<li title="Page ${i}"><a
								href="${CONTEXT_PATH}/dead/inmormantari/${i}?order=${order}&sch=${sch}">${i}</a></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				...
				<c:choose>
					<c:when test="${currentPage lt nrOfPages}">
						<li title="Next page"><a
							href="${CONTEXT_PATH}/dead/inmormantari/${currentPage + 1}?order=${order}&sch=${sch}">Next</a></li>

						<li title="Last page"><a
							href="${CONTEXT_PATH}/dead/inmormantari/${nrOfPages}?order=${order}&sch=${sch}">
								Last </a></li>
					</c:when>
					<c:otherwise>
						<li class="disabled" title="Next page"><a
							href="${CONTEXT_PATH}/dead/inmormantari/${nrOfPages}?order=${order}&sch=${sch}">Next</a></li>
